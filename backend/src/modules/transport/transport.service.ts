import { Injectable, NotFoundException } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { TransportRoute } from './transport.entity';
import { PaginationDto } from '../../common/dto/pagination.dto';
import { paginate } from '../../common/utils/pagination.util';

@Injectable()
export class TransportService {
  constructor(@InjectRepository(TransportRoute) private repo: Repository<TransportRoute>) {}

  create(data: Partial<TransportRoute>, uid?: string) {
    return this.repo.save(this.repo.create({ ...data, created_by: uid }));
  }

  findAll(dto: PaginationDto, branchId?: string) {
    const q = this.repo.createQueryBuilder('r').where('r.is_deleted = false');
    if (branchId) q.andWhere('r.branch_id = :branchId', { branchId });
    if (dto.search) q.andWhere('r.route_name ILIKE :s OR r.route_code ILIKE :s OR r.vehicle_no ILIKE :s OR r.driver_name ILIKE :s', { s: `%${dto.search}%` });
    q.orderBy('r.route_name', 'ASC');
    return paginate(q, dto);
  }

  async findById(id: string) {
    const r = await this.repo.findOne({ where: { id, is_deleted: false } });
    if (!r) throw new NotFoundException(`Route ${id} not found`);
    return r;
  }

  async update(id: string, data: Partial<TransportRoute>, uid?: string) {
    const r = await this.findById(id);
    return this.repo.save(Object.assign(r, data, { updated_by: uid }));
  }

  async remove(id: string, uid?: string) {
    const r = await this.findById(id);
    r.is_deleted = true; r.deleted_at = new Date(); r.updated_by = uid;
    await this.repo.save(r);
    return { message: 'Route removed' };
  }

  async getStats(branchId: string) {
    const totalRoutes = await this.repo.count({ where: { branch_id: branchId, is_deleted: false } });
    const result = await this.repo
      .createQueryBuilder('r')
      .select('SUM(r.capacity)', 'totalCapacity')
      .where('r.branch_id = :branchId AND r.is_deleted = false', { branchId })
      .getRawOne();
    return { totalRoutes, totalCapacity: +result.totalCapacity || 0 };
  }
}
